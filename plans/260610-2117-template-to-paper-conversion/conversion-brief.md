...
> ⚠️ **12/06**: Paper đã đổi cách cấp ID (artboard mới ra dạng `9Y-0`/`H5-0`...). Node-id con `2-0/9-0/93-0` trong home có thể đã lệch — BẮT BUỘC `get_children` trên `1-0` lấy ID top strip/nav/footer thật trước khi clone, đừng tin cứng số cũ.
...
| products.html | ✅ | "QS Sản phẩm" — clone top strip/nav/footer OK; nav "Sản phẩm" active (gold). Hero grid 2 cột dùng flexBasis 42%/54% (né pitfall flex-grow). Body = sidebar flexBasis 240px + main flex-grow:1. 6 card-row dựng 1 lần rồi duplicate_nodes + set_text_content (cột badge/img/info/acc lane dọc thẳng). Mockup controller + img thumbnail tái dựng bằng div/rectangle (không paste SVG). Tag [Touch]/[Flagship] đổi màu xanh #3a8d4d. |
| product-detail.html | ✅ | "QS Chi tiết sản phẩm" — clone top strip/nav/footer OK; nav "Sản phẩm" active (gold). Hero grid 2 cột flexBasis 46%/50%; controller mockup + 3 gallery view + I/O board tái dựng bằng div/rectangle (không paste SVG). Spec table = 2 cột flexBasis 50% (mỗi cột 4 row), row key/value dùng duplicate_nodes + set_text_content. Sidebar flexBasis 240px + main flex-grow:1. Package diagram dựng 3 lane (Pkg node duplicate ×5); bỏ đường connector dọc gốc → đọc dạng hub-spoke, chấp nhận được. |
| applications.html | ✅ | "QS Ứng dụng" — clone top strip/nav/footer OK; nav "Ứng dụng" active (gold). Hero grid 2 cột flexBasis 42%/54%; PCB board tái dựng bằng div/rectangle (chip QS center + 3 chip phụ + caps, không paste SVG). Intro grid 32%/54%. Grid 4 cột: card 01 dựng 1 lần rồi duplicate_nodes ×7 + set_text_content; LƯU Ý card dùng flexBasis 25% KHÔNG fill full-width (gap hairline) → phải đổi flexGrow:1 + flexBasis:0% cho 8 card thì mới đều 4 cột. Card đặc biệt "+07 more" nền dark. Minh hoạ máy tái dựng bằng rectangle, thêm accent gold cho vài card. |
| application-detail.html | ✅ | "QS Chi tiết ứng dụng" — clone top strip/nav/footer OK; nav "Ứng dụng" active (gold). Hero dark 2 cột flexBasis 56%/40%, máy phay mockup + stats lane tái dựng bằng div/rectangle (không paste SVG). Process 4 card hairline dùng flexGrow:1+flexBasis:0% (đều full-width), duplicate ×3 + set_text_content; icon dùng 1 shape chung. Spec 2 cột 44%/52%; iso part dựng bằng clip-path hexagon + bore + title block; spec-table 7 row key 200px slot + value flex-grow, duplicate ×6. Gallery dark 3 card flexBasis:0% + Related 3 card hairline cùng pattern duplicate. |
| service.html | ✅ | "QS Dịch vụ" — clone top strip/nav/footer OK; nav "Dịch vụ" active (gold). Hero meta-stats 4 cột dùng flexGrow:1+flexBasis:0% (fill full-width, divider phải bỏ ở cột cuối). Pitch 2 cột flexBasis 40% + flexGrow:1; câu có `<strong>` render đồng màu (1 text=1 màu, chấp nhận). Capabilities 3 card hairline flexGrow:1+flexBasis:0%, icn PCB tái dựng bằng div/rectangle (không paste SVG), duplicate ×2 + set_text_content. Process timeline dark: dashed top border + 5 step flexGrow:1, node marker absolute, duplicate ×4. CTA/form 2 cột 50/50; form input/select/textarea = div placeholder (static mockup, không field thật); contact row key slot 120px. |
| service-detail.html | ✅ | "QS Chi tiết dịch vụ" — clone top strip/nav/footer OK; nav "Dịch vụ" active (gold). Hero 2 cột flexGrow 1.2/1, "máy mới" tách element gold solid, before/after controller mockup tái dựng bằng div/rectangle (không paste SVG). Process timeline 5 step flexGrow:1+flexBasis:0% (đều full-width), step 1–3 active num dark, 4–5 light, duplicate ×4 + set_text_content. Incl table 9 row check/name/tag lane dọc (slot 22px + tag flexShrink:0), 2 row cuối "no" check paper-2 muted, duplicate ×8. Pricing 3 gói: card dựng 1 lần rồi duplicate ×2; LƯU Ý flexGrow:1+flexBasis:0% KHÔNG fill full-width → phải set flexBasis 33.3% cho 3 card. Pro featured restyle dark + gold accents. FAQ 2×2: flexGrow:0+flexShrink:0+flexBasis calc(50% - 0.5px) mới đúng 2 cột (flexGrow:1 làm card đầu nuốt cả hàng). CTA dark box 2 cột. |
| about.html | ✅ | "QS Giới thiệu" — clone top strip/nav/footer OK; không set nav active đúng mapping. Hero 2 cột flexBasis 56%/39%, chữ "controller" tách node màu gold để né inline rich-text. Story 30%/main copy, timeline 6 mốc dùng duplicate_nodes + set_text_content. Values 3 card duplicate, facility dark có plant diagram tái dựng bằng div/rectangle + specs 6 row, team 4 card duplicate, CTA dark box. |
| news.html | ✅ | "QS Tin tức" — clone top strip/nav/footer OK; nav "Tin tức" active (gold). Head 2 cột title/meta, tabs sticky mockup, featured card 1.3fr/1fr với Astro 12X visual tái dựng bằng div/rectangle. Article grid 6 card duplicate_nodes + set_text_content; LƯU Ý flexBasis calc làm rơi 1 cột → sửa bằng flexBasis/width 31.8% để đúng 3 cột. Pagination static buttons, newsletter dark 2 cột + form mockup. |
| news-detail.html | ✅ | "QS Chi tiết tin tức" — clone top strip/nav/footer OK; nav "Tin tức" active (gold). Breadcrumb + article head 1fr/320px meta card. Hero image Astro 12X tái dựng bằng div/rectangle (controller body, display, keypad, IO blocks, emergency button) + caption dark gradient. Body = article max 760px + TOC 240px; quote box, spec list duplicate_nodes + set_text_content với bullet lane cố định, delivery timeline figure dựng bằng shapes. Tags row + related 3 card duplicate, fit-content OK. |
| case-studies.html | ✅ | "QS Case Studies" — clone top strip/nav/footer OK; không set nav active đúng mapping. Hero 2 cột với "14" tách node gold, stats dark 4 cột dùng flexBasis 25% (sửa lỗi vùng trống do flex grow), filter chips static. Featured case 1.4fr/1fr với controller mockup tái dựng bằng div/rectangle + nums 3 cột. Case grid 9 card duplicate_nodes + set_text_content, flexBasis 33.25% để giữ 3 cột hairline. Logo wall 12 cell width 16.55% + CTA dark box. |
| case-study-detail.html | ✅ | "QS Chi tiết case study" — clone top strip/nav/footer OK; nav không active đúng mapping. Dark hero + breadcrumb/stamp, H1 F86 gold, meta 4 cell, cover Precitech line mockup, outcome stats 4 cell, body 3 cột TOC/article/sidebar, related 3 card, fit-content OK. |
| support.html | ✅ | "QS Hỗ trợ kỹ thuật" — clone top strip/nav/footer OK; nav không active đúng mapping. Hero sáng + live badge, channels 4 card, ticket form/checklist, system status dark 4 card, resources 3 card, fit-content OK. |
| faq.html | ✅ | "QS FAQ" — clone top strip/nav/footer OK; nav không active đúng mapping. Hero/search, TOC trái, article FAQ 5 nhóm với accordion open/collapsed, CTA ticket, fit-content OK. |
| downloads.html | ✅ | "QS Tải tài liệu" — clone top strip/nav/footer OK; nav không active đúng mapping. Hero search + quick links + document stack shape, category grid 4 card, featured dark 3 item, help cards 2 cột, fit-content OK. |
| downloads-list.html | ✅ | "QS Danh sách tài liệu" — clone top strip/nav/footer OK; nav không active đúng mapping. Hero dark + stats, sidebar filters, toolbar chips/sort/view, table 8 rows with fixed lanes, pager, fit-content OK. |
| careers.html | ✅ | "QS Tuyển dụng" — clone top strip/nav/footer OK; nav không active đúng mapping. Hero dark + stat stamp, values 4 card, benefits compact, jobs list/filter, life gallery, quote + CV CTA, fit-content OK. |
| search.html | ✅ | "QS Kết quả tìm kiếm" — clone top strip/nav/footer OK; nav không active đúng mapping. Hero/search input/tabs, results list 6 row, sidebar filters/recent/help, pager, fit-content OK. |
