  ...
  return (
    ...
    >
      ...
      >
        ...
        {lead.badge && (
          <span className="absolute top-7 left-7 font-mono text-[10px] text-gold-2 tracking-[.2em] uppercase">[ {lead.badge} ]</span>
        )}
        <div className="absolute inset-x-0 bottom-0 p-8 lg:p-10">
          <h3 className="font-display font-semibold text-white text-[26px] lg:text-[32px] leading-[1.12] tracking-[-.01em] max-w-[24ch]">{lead.title}</h3>
          <p className="text-[#d2ccba] text-sm leading-[1.6] mt-3.5 max-w-[52ch]">{lead.desc}</p>
          <div className="mt-5 font-mono text-[10px] text-gold-2 tracking-[.14em]">{lead.date} · {lead.read}</div>
        </div>
      </Link>
        ...
        {items.map((n, i) => (
          ...
          >
            ...
            <div className="min-w-0">
              <span className="font-mono text-[9px] text-muted tracking-[.16em] uppercase block mb-1">{n.cat} · {n.date}</span>
              <h4 className="font-display font-semibold text-[14px] leading-[1.35] m-0 tracking-[-.005em] text-ink line-clamp-2 transition-colors group-hover:text-gold-1 group-data-[active=true]:text-gold-1">{n.title}</h4>
            </div>
