"use client";

import { useEffect, useState } from "react";

/* Floating contact rail — fixed bottom-right quick-contact buttons + back-to-top.
   Contact buttons use official brand marks/colors (Zalo, WhatsApp) + a hotline
   call button; back-to-top keeps the QS gold gradient.
   Static-export safe: client component, real anchors, no dynamic server APIs.
   Primary number 0909663350 powers Zalo / phone / WhatsApp. */

const PRIMARY_PHONE_LOCAL = "0909663350";
const PRIMARY_PHONE_INTL = "84909663350"; // E.164 without '+', for wa.me / zalo

type ContactItem = {
  label: string;
  href: string;
  external?: boolean;
  bg: string; // brand background color
  ring: string; // brand-tinted attention-pulse color
  glyph: React.ReactNode;
};

/* Generic phone handset — hotline call button (no third-party brand). */
const phoneGlyph = (
  <svg width="19" height="19" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
    <path d="M6.6 10.8a15.5 15.5 0 0 0 6.6 6.6l2.2-2.2a1 1 0 0 1 1-.24c1.1.37 2.3.57 3.6.57a1 1 0 0 1 1 1V20a1 1 0 0 1-1 1A17 17 0 0 1 3 4a1 1 0 0 1 1-1h3.5a1 1 0 0 1 1 1c0 1.3.2 2.5.57 3.6a1 1 0 0 1-.25 1l-2.22 2.2z" />
  </svg>
);

/* Official Zalo mark (Simple Icons). */
const zaloGlyph = (
  <svg width="23" height="23" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
    <path d="M12.49 10.2722v-.4496h1.3467v6.3218h-.7704a.576.576 0 01-.5763-.5729l-.0006.0005a3.273 3.273 0 01-1.9372.6321c-1.8138 0-3.2844-1.4697-3.2844-3.2823 0-1.8125 1.4706-3.2822 3.2844-3.2822a3.273 3.273 0 011.9372.6321l.0006.0005zM6.9188 7.7896v.205c0 .3823-.051.6944-.2995 1.0605l-.03.0343c-.0542.0615-.1815.206-.2421.2843L2.024 14.8h4.8948v.7682a.5764.5764 0 01-.5767.5761H0v-.3622c0-.4436.1102-.6414.2495-.8476L4.8582 9.23H.1922V7.7896h6.7266zm8.5513 8.3548a.4805.4805 0 01-.4803-.4798v-7.875h1.4416v8.3548H15.47zM20.6934 9.6C22.52 9.6 24 11.0807 24 12.9044c0 1.8252-1.4801 3.306-3.3066 3.306-1.8264 0-3.3066-1.4808-3.3066-3.306 0-1.8237 1.4802-3.3044 3.3066-3.3044zm-10.1412 5.253c1.0675 0 1.9324-.8645 1.9324-1.9312 0-1.065-.865-1.9295-1.9324-1.9295s-1.9324.8644-1.9324 1.9295c0 1.0667.865 1.9312 1.9324 1.9312zm10.1412-.0033c1.0737 0 1.945-.8707 1.945-1.9453 0-1.073-.8713-1.9436-1.945-1.9436-1.0753 0-1.945.8706-1.945 1.9436 0 1.0746.8697 1.9453 1.945 1.9453z" />
  </svg>
);

/* Official WhatsApp mark (Simple Icons). */
const whatsappGlyph = (
  <svg width="19" height="19" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
    <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
  </svg>
);

const chevronUpGlyph = (
  <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.4" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
    <path d="m6 15 6-6 6 6" />
  </svg>
);

const contacts: ContactItem[] = [
  { label: "Gọi điện", href: `tel:+${PRIMARY_PHONE_INTL}`, bg: "bg-[#e23744]", ring: "rgba(226,55,68,.5)", glyph: phoneGlyph },
  { label: "Zalo", href: `https://zalo.me/${PRIMARY_PHONE_LOCAL}`, external: true, bg: "bg-[#0068ff]", ring: "rgba(0,104,255,.45)", glyph: zaloGlyph },
  { label: "WhatsApp", href: `https://wa.me/${PRIMARY_PHONE_INTL}`, external: true, bg: "bg-[#25d366]", ring: "rgba(37,211,102,.5)", glyph: whatsappGlyph },
];

/* Shared circular badge geometry + interaction; color comes from the caller. */
const BADGE =
  "group/fc relative grid h-12 w-12 place-items-center rounded-full shadow-[0_8px_22px_-8px_rgba(10,8,6,.7)] ring-1 ring-black/10 transition-[transform,opacity] duration-200 hover:scale-110 focus-visible:scale-110 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-gold-2";

function Label({ children }: { children: React.ReactNode }) {
  return (
    <span className="pointer-events-none absolute right-full mr-3 whitespace-nowrap rounded-md bg-[#1a1206] px-2.5 py-1 text-[12px] font-medium text-gold-2 opacity-0 shadow-lg transition-opacity duration-200 group-hover/fc:opacity-100">
      {children}
    </span>
  );
}

export default function FloatingContact() {
  const [showTop, setShowTop] = useState(false);

  useEffect(() => {
    const onScroll = () => setShowTop(window.scrollY > 480);
    onScroll();
    window.addEventListener("scroll", onScroll, { passive: true });
    return () => window.removeEventListener("scroll", onScroll);
  }, []);

  return (
    <div className="fixed bottom-5 right-4 z-30 flex flex-col items-center gap-[18px] sm:right-5">
      {contacts.map((c, i) => (
        <a
          key={c.label}
          href={c.href}
          aria-label={c.label}
          className={`${BADGE} ${c.bg} text-white`}
          {...(c.external ? { target: "_blank", rel: "noopener noreferrer" } : {})}
        >
          {/* Brand-tinted attention pulse — behind the glyph, staggered per item. */}
          <span
            aria-hidden="true"
            className="qs-fc-pulse pointer-events-none absolute inset-0 rounded-full"
            style={{ "--fc-ring": c.ring, animationDelay: `${i * 0.45}s` } as React.CSSProperties}
          />
          <Label>{c.label}</Label>
          {c.glyph}
        </a>
      ))}

      <button
        type="button"
        onClick={() => window.scrollTo({ top: 0, behavior: "smooth" })}
        aria-label="Lên đầu trang"
        className={`${BADGE} bg-gold-grad text-[#1a1206] ${showTop ? "opacity-100" : "pointer-events-none translate-y-2 opacity-0"}`}
      >
        <Label>Lên đầu trang</Label>
        {chevronUpGlyph}
      </button>
    </div>
  );
}
