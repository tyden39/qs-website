...
export async function generateMetadata({
  ...
  // The locale-aware copy in messages wins; the raw seed (Vietnamese-only) is
  // the fallback so `en` metadata never regresses to Vietnamese.
...
export default async function ServiceDetail({ params }: { params: Promise<{ locale: Locale; slug: string }> }) {
  ...
  // Structured data mirrors the copy actually rendered above, so JSON-LD stays
  // in the page's language instead of the Vietnamese-only seed.
  const view = getServiceBySlug(slug, locale);
  const serviceJsonLd = view
    ? buildService({ ...view, title: s.name, hero: { ...view.hero, subhead: s.lede } }, locale)
    : null;
          ...
          <div className="grid md:grid-cols-[1.2fr_1fr] gap-10 sm:gap-16 items-start">
            <div className="order-2 md:order-1">
              ...
              <div className="mt-9 grid grid-cols-3 gap-4 sm:gap-8 pt-6 border-t border-line">
                {s.stats.map(([l, v]) => (
                  <div key={l} className="font-mono text-label-xs text-muted tracking-[.16em] uppercase">
                    {l}
                    <b className="block font-display text-title sm:text-subhead font-bold tracking-[-.01em] normal-case text-ink mt-1">{v}</b>
                  </div>
                ))}
              </div>
            </div>

            <div className="relative order-1 aspect-[5/6] bg-white border border-line p-[18px] md:order-2">
              ...
              <div className="absolute left-[18px] bottom-3.5 font-mono text-label-xs tracking-[.16em] text-muted">FIG. 01 · RETROFIT · BEFORE / AFTER</div>
            </div>
          </div>
        </div>
      </section>
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-px bg-line border border-line relative">
            {s.process.map(step => (
              <div key={step.num} className="bg-white px-[22px] py-7 flex flex-col gap-2.5 relative z-[2]">
                <div className={`w-10 h-10 grid place-items-center font-display font-bold text-title mb-2
                                 ${step.active ? "bg-ink text-gold-2 border-2 border-ink" : "bg-white text-ink border-2 border-ink"}`}>
                  {step.num}
                </div>
                <span className="font-mono text-label-xs text-gold-1 tracking-[.14em] uppercase">{step.day}</span>
                <h3 className="font-display text-lede font-semibold leading-[1.3] tracking-[-.005em] m-0 mt-1">{step.title}</h3>
                <p className="text-[#4a4842] text-meta leading-[1.6] m-0 mt-1.5">{step.desc}</p>
                <div className="font-mono text-label-xs text-muted tracking-[.1em] pt-3 border-t border-line mt-auto">{step.duration}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="max-w-wrap mx-auto px-5 sm:px-8 lg:px-12 grid md:grid-cols-[1fr_1.2fr] gap-10 sm:gap-16 items-start">
          <div>
            ...
            {s.includesIntro.map((p, i) => (
              <p key={i} className="text-body leading-[1.75] text-[#3a3a3a] m-0 mb-4">{p}</p>
            ))}
          </div>
          <div className="bg-white border border-line">
            {s.includes.map((item, i) => (
              <div key={i} className={`grid grid-cols-[auto_1fr_auto] gap-[18px] px-[22px] py-[18px] items-center
                ...
                <div className={`w-[22px] h-[22px] grid place-items-center text-meta shrink-0
          ...
          <div className="grid md:grid-cols-3 gap-px bg-line border border-line">
            {s.packages.map(pkg => (
              <div key={pkg.title}
                ...
                <span className={`font-mono text-label tracking-[.16em] ${pkg.featured ? "text-gold-2" : "text-gold-1"}`}>{pkg.name}</span>
                <h3 className={`font-display text-subhead font-bold tracking-[-.01em] m-0 ${pkg.featured ? "text-white" : ""}`}>{pkg.title}</h3>
                <div className={`py-3.5 border-b ${pkg.featured ? "border-[#2a2620]" : "border-line"}`}>
                  <b className={`font-display text-h2 font-bold tracking-[-.015em] ${pkg.featured ? "text-white" : "text-ink"}`}>{pkg.price}</b>
                  <small className={`block text-meta mt-1 ${pkg.featured ? "text-[#a8a499]" : "text-muted"}`}>{pkg.priceNote}</small>
                </div>
                <ul className="list-none p-0 m-0 flex flex-col gap-2 flex-1">
                  {pkg.features.map(f => (
                    <li key={f} className={`text-meta leading-[1.55] pl-[22px] relative
                                            ${pkg.featured ? "text-[#cfc9b8] before:text-gold-2" : "text-[#2a2a2a] before:text-gold-1"}
                                            before:content-['✓'] before:absolute before:left-0 before:top-0 before:font-bold before:text-meta`}>
                      {f}
                    </li>
                  ))}
                </ul>
